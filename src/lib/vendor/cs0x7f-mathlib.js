// vendored from https://github.com/cs0x7f/cstimer/blob/master/src/js/lib/mathlib.js
// Converted to an ESM default export. The only logic change is providing a default-false
// `DEBUG` global (cstimer expects it to exist on `window` and gates console.log on it).
"use strict";

// cstimer references a global DEBUG flag for verbose logging. Define it as false here so
// we don't get ReferenceError; if you want verbose solver output, set globalThis.DEBUG=true.
const DEBUG = (typeof globalThis !== 'undefined' && globalThis.DEBUG) || false;

// cstimer also expects a global `isaac` — its ISAAC cryptographic PRNG. We don't need
// crypto-grade randomness for solving (we're not generating WCA-legal random scrambles
// here), so we stub it with Math.random. The solver uses the rng only inside random
// scramble generation paths we don't call.
const isaac = {
	seed: () => {},
	random: () => Math.random(),
};
